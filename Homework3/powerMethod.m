function [evalue, evector] = powerMethod(A, atol)
...
for i = 2:200                                                               % we iterate from 2 to 100
    z(:,i) = A*Q(:,i-1);                                                    % we find z_i by multiplying A with q_i                        
    Q(:,i) = z(:,i)/ norm(z(:,i));                                          % the q_i is z_i / the norm of z_i
    lambda(1,i) = Q(:,i)' * A * Q(:,i);                                      % lambda_i  is now q_i' times A times q_i
    table(i,1) = i;                                                         % table first column showing iteration number
    table(i,3) = norm(Q(:,i)-Q(:,i-1));                                     % table third column showing norm of difference
    table(i,4) = lambda(1,i);                                               % table column showing the eigenvalue
    table(i,2) = norm(A*Q(:,i) - (Q(:,i)' * A * Q(:,i)* Q(:,i)));           % table showing rayleigh quotient residual norm
    
    if norm(Q(:,i)-Q(:,i-1)) < atol                                         % this is the condition for breaking out
        evalue = lambda(i);                                                 % we output evalue from the last lambda(i)
...
disp('Computation timed out, possibly no dominant eigenvalue');             % this is the computation time-out
evalue = lambda(end);
evector = Q(:,end);
